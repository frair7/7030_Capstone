...
def normalize_orig_frame(frame_orig):
    """
    Normalize the original Frame column (IF/OOF/Unk/N/A/(blank)/etc.)
    to 'in' / 'out' / None for comparison against the light table.
    """
    if not frame_orig:
        return None
...
def main():
        ...
        for row in reader:
            ...
            if start is None:
                ...
                results.append([
                    ...
                    "N/A", "N/A", *["" for _ in range(args.top)], note
                ])
                continue
            if is_approx:
            ...
            expected_frame = normalize_orig_frame(frame_orig)
            ...
            # Add clarifying notes on top of any existing note (e.g. approx breakpoint)
            extra_notes = []
            if is_already_in_frame(mtype_rows, start, end):
    ...
    header = (
        ...
        + [f"Skip_{i+1}" for i in range(args.top)]
        + ["Notes"]
    )
